// ============================================================
// tesinversion.com — Dashboard Connect v2.0
// Integración del dashboard con datos reales de FMP API
// Incluir DESPUÉS de fmp-api.js y ANTES del cierre de </body>
//
// v2.0: corregidos nombres de campos para alinear con fmp-api.js
//   rev (no revenue), ni (no netIncome), netPos (no netPosition)
//   mktSec (no marketSec), shares (no dilutedShares)
//   Añadido: reinvRate, cálculo correcto de deuda neta
// ============================================================
...
(async function initDashboard() {
  ...
  fillRedFlags(data);
  fillChartsFromData(data);  // ← actualiza todos los gráficos con datos reales
  ...
  function fillHeader(d) {
    ...
    setStyle('[data-field="change"]', 'color', d.changePct >= 0 ? 'var(--green)' : 'var(--red)');
    setText('[data-field="market-cap"]',    FMP.fmt.big(d.marketCap));
    const l = d.last;
    setText('[data-field="net-position"]',
      l.netPos < 0
        ? `+${FMP.fmt.big(Math.abs(l.netPos))} caja neta`
        : FMP.fmt.big(l.netPos) + ' deuda neta');
    setStyle('[data-field="net-position"]', 'color', l.netPos < 0 ? 'var(--green)' : 'var(--red)');
    setText('[data-field="eps"]',           FMP.fmt.price(l.eps));
  ...
  function fillResumen(d) {
    ...
    setMetricTile('revenue',        FMP.fmt.big(l.rev),      pctChange(d, 'rev'));
    setMetricTile('ebit',           FMP.fmt.big(l.ebit),     `Margen: ${FMP.fmt.pct(l.ebitMargin)}`);
    setMetricTile('net-income',     FMP.fmt.big(l.ni),       `Margen: ${FMP.fmt.pct(l.netMargin)}`);
    ...
    setMetricTile('per',            FMP.fmt.mult(l.per),     'Precio / EPS diluido');
  ...
  function fillFinancieros(d) {
    ...
    setFinRow('revenue',       rows.map(r => FMP.fmt.big(r.rev)));
    setFinRow('gross-profit',  rows.map(r => FMP.fmt.big(r.gp)));
    ...
    setFinRow('net-income',    rows.map(r => FMP.fmt.big(r.ni)));
  ...
  function fillRetornoCapital(d) {
    ...
    setText('[data-field="roic-incremental"]',
      FMP.fmt.pct(d.roicIncremental[d.roicIncremental.length - 1]));
  ...
  function fillSaludFinanciera(d) {
    ...
    setText('[data-field="cash-total"]',    FMP.fmt.big(l.cash + l.mktSec));
    setText('[data-field="net-position"]',
      l.netPos < 0
        ? `+${FMP.fmt.big(Math.abs(l.netPos))} caja neta`
        : FMP.fmt.big(l.netPos) + ' deuda neta');
    ...
    // Tacómetros (si existen en el HTML)
  ...
  function fillCapitalAllocation(d) {
    ...
    updateChartData('sharesChart',  d.years, d.computed.map(r => r.shares / 1e6));
  }
  ...
  function fillScorecard(d) {
    ...
    Object.entries(scores).forEach(([key, val]) => {
      ...
      if (pts) {
        pts.textContent = val.toFixed(1) + '/10';
        pts.className   = 'score-pts ' + (val >= 8 ? 'high' : val >= 6 ? 'mid' : 'low');
      }
    });
  }
  ...
  function fillGuidance(d) {
    ...
    tbody.innerHTML = d.earningsHistory.slice(0, 8).map(e => {
      ...
      return `<tr>
        ...
        <td><span class="${badgeCls}">${beat ? 'Beat' : 'Miss'}</span></td>
    ...
    setText('[data-field="guidance-rate"]',
      total > 0 ? ((beats / total) * 100).toFixed(0) + '%' : '—');
  }
  ...
  function fillAccionistas(d) {
    ...
    wrap.innerHTML = d.topInstitutional.map(s => `
      <div class="sh-row">
        ...
        <div class="sh-bar-bg">
          <div class="sh-bar" style="width:${Math.min(100, s.pct * 4)}%;background:var(--green)"></div>
        </div>
        <span class="sh-pct">${FMP.fmt.pct(s.pct, 1)}</span>
      </div>`).join('');

    updateChartData('sharesHistChart', d.years, d.computed.map(r => r.shares / 1e6));
  }
  ...
  function fillRedFlags(d) {
    ...
    // ROIC
    updateFlag('flag-roic',
      l.roic >= 15 ? 'green' : l.roic >= 8 ? 'yellow' : 'red',
    ...
    // Deuda
    updateFlag('flag-debt',
      l.netPos < 0 ? 'green' : l.debtEbitda < 2 ? 'green' : l.debtEbitda < 4 ? 'yellow' : 'red',
      l.netPos < 0
        ? `Caja neta ${FMP.fmt.big(Math.abs(l.netPos))}`
        : `Deuda neta / EBITDA ${FMP.fmt.mult(l.debtEbitda)}`,
      l.netPos < 0   ? 'Posición de caja neta — fortaleza financiera excepcional.' :
      l.debtEbitda < 2 ? 'Deuda controlada, por debajo del umbral de calidad (2x).' :
                         'Deuda elevada. Revisar capacidad de cobertura.');

    // Revenue
    updateFlag('flag-revenue',
      flags.revDecline === 0 ? 'green' : flags.revDecline <= 1 ? 'yellow' : 'red',
      flags.revDecline === 0
        ? `Revenue creciendo todos los años`
        : `Revenue declinó ${flags.revDecline} año(s) en los últimos 5`,
      flags.revDecline === 0
        ? 'Crecimiento de ingresos consistente — señal de demanda sostenida.'
        : 'Verificar si la caída es cíclica o estructural.');

    // FCF
    updateFlag('flag-fcf',
      l.fcfConv >= 80 ? 'green' : l.fcfConv >= 60 ? 'yellow' : 'red',
      ...
      l.fcfConv >= 60 ? 'FCF conversion moderada. Verificar si hay partidas no recurrentes.' :
                        'FCF por debajo del umbral de calidad (60%).');

    // Dilución
    const lastShares  = d.computed[d.computed.length - 1]?.shares;
    const firstShares = d.computed[0]?.shares;
    const dilution5y  = firstShares ? ((lastShares - firstShares) / firstShares) * 100 : null;
    updateFlag('flag-dilution',
      dilution5y < 0 ? 'green' : dilution5y < 3 ? 'yellow' : 'red',
      dilution5y < 0
        ? `Recompras netas: ${FMP.fmt.pct(dilution5y)}`
        : `Dilución neta: +${FMP.fmt.pct(dilution5y)}`,
      dilution5y < 0
        ? 'Reducción neta del share count. Management alineado con accionistas.'
        : 'Share count creciendo. Verificar si recompras cubren las stock options.');
  }

  // ── ACTUALIZAR GRÁFICOS CON DATOS REALES ─────────────────
  // Esta función actualiza los gráficos que fueron inicializados
  // con datos hardcodeados en el HTML. Los datos reales del API
  // (con overrides aplicados) reemplazan los datos de placeholder.
  function fillChartsFromData(d) {
    const yrs = d.years;
    const M   = 1e6;  // millones

    // Revenue y FCF (en miles de millones)
    updateChartData('tRevFcf',   yrs,
      null,
      [d.computed.map(r => r.rev  / 1e9),
       d.computed.map(r => r.fcf  / 1e9)]);

    // ROIC histórico + incremental
    updateChartData('roicChart', yrs,
      null,
      [d.computed.map(r => r.roic),
       d.roicIncremental]);

    // Márgenes
    updateChartData('marginsChart', yrs,
      null,
      [d.computed.map(r => r.grossMargin),
       d.computed.map(r => r.ebitMargin),
       d.computed.map(r => r.netMargin),
       d.computed.map(r => r.fcfMargin)]);

    // Moat (margen EBIT vs sector)
    updateChartData('moatChart', yrs,
      null,
      [d.computed.map(r => r.ebitMargin)]);

    // EPS
    updateChartData('tEps', yrs,
      null,
      [d.computed.map(r => r.eps)]);

    // Múltiplos históricos (solo si tienen precio histórico)
    updateChartData('tMultiplos', yrs,
      null,
      [d.computed.map(r => r.per),
       d.computed.map(r => r.evEbitda),
       d.computed.map(r => r.evFcf)]);

    // Capital Allocation
    if (d.capitalAllocation) {
      updateCapitalAllocChartFromData(d);
    }

    // Tendencias: deuda neta histórica
    updateChartData('debtEvChart', yrs,
      null,
      [d.computed.map(r => r.netPos / M)]);

    // ROIC chart en sección Retorno Capital
    updateChartData('roicHistChart', yrs,
      null,
      [d.computed.map(r => r.roic)]);
    updateChartData('roicIncrChart', yrs,
      null,
      [d.roicIncremental]);
  }

  function updateCapitalAllocChartFromData(d) {
    const inst = window.chartInstances && window.chartInstances['capAlloc'];
    if (!inst) return;
    const M = 1e6;
    const alloc = d.capitalAllocation;
    inst.data.labels = d.years;
    inst.data.datasets[0].data = alloc.map(r => r.dividends  / M);
    inst.data.datasets[1].data = alloc.map(r => r.buybacks   / M);
    inst.data.datasets[2].data = alloc.map(r => r.growthCapex/ M);
    inst.data.datasets[3].data = alloc.map(r => r.debtRepay  / M);
    inst.data.datasets[4].data = alloc.map(r => r.cashAccum  / M);
    inst.update('none');
  }

  // ── CÁLCULO DEL ÍNDICE DE CALIDAD ────────────────────────
  // 10 criterios × 10 puntos = 100 máximo
  // Basado en fórmulas de tesinversion.com / FORMULAS.docx
  function calcQualityScoreBreakdown(d) {
    const l   = d.last;
    const avg = (key, n=5) => avg5(d, key, n);

    // 1. ROIC consistente >15% (media mercado 13%)
    ...
    // 3. Salud financiera (deuda neta / EBITDA — media mercado 2x)
    const s3 = l.netPos < 0 ? 10 : l.debtEbitda < 1 ? 9 : l.debtEbitda < 2 ? 7 :
               l.debtEbitda < 3 ? 5 : l.debtEbitda < 4 ? 3 : 1;

    // 4. FCF conversion >80%
    const s4 = l.fcfConv >= 90 ? 10 : l.fcfConv >= 80 ? 8.5 : l.fcfConv >= 70 ? 6.5 :
               l.fcfConv >= 60 ? 5 : 3;

    // 5. Capital allocation: dilución vs recompras
    const shares = d.computed.slice(-5).map(r => r.shares);
    ...
    // 7. ROIC incremental vs WACC estimado (8% = tasa libre de riesgo + prima)
    const wacc     = 8;
    ...
    // 8. Guidance accuracy (earnings beats)
    ...
    const s8 = beatPct >= 0.85 ? 10 : beatPct >= 0.75 ? 8 : beatPct >= 0.65 ? 6 :
               beatPct >= 0.5 ? 4 : 2;

    // 9. Insider activity (señal de confianza del management)
    const insiderBuys = d.recentInsiders.filter(t => t.isBuy).length;
    const s9 = insiderBuys >= 3 ? 10 : insiderBuys >= 2 ? 8 : insiderBuys >= 1 ? 6 : 4;

    // 10. Dilución neta 5 años
    const s10 = dilNet < -8 ? 10 : dilNet < -3 ? 8.5 : dilNet < 0 ? 7 :
                dilNet < 2  ? 5  : dilNet < 5   ? 3   : 1;
  ...
  // Basado en metodología de valoración de tesinversion.com
  function calcPrice15(d) {
    const fcfGrowth  = 0.12;
    const exitMult   = d.last.evFcf || 44;
    const shares     = d.last.shares;
    const netPos     = d.last.netPos;
  ...
  // Actualiza un chart existente en window.chartInstances
  // Soporta múltiples datasets: si datasets[] se pasa, actualiza todos
  function updateChartData(chartId, labels, singleData, datasets) {
    const inst = window.chartInstances && window.chartInstances[chartId];
    if (!inst) return;
    ...
    if (datasets && datasets.length) {
      datasets.forEach((data, i) => {
        if (inst.data.datasets[i] && data) {
          inst.data.datasets[i].data = data;
        }
      });
    } else if (singleData) {
      inst.data.datasets[0].data = singleData;
    }
  ...
  function showError(msg) {
    ...
    else console.error('[dashboard-connect]', msg);
  }
